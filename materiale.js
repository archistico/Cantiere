...
var Materiali = (function () {
    ...
    Materiali.prototype.righeTabella = function () {
        var risultato = "";
        for (var c = 0; c < this.lista.length; c++) {
            risultato += "<tr><td scope='row'>" + this.lista[c].quantita + "</th><td>" + MaterialeTipo[this.lista[c].tipologia] + "</td><td>" + this.lista[c].costo + " €</td><td><button>-</button><button onclick='compraMateriale(" + MaterialeTipo[this.lista[c].tipologia].toLowerCase() + ")'>+</button></td></tr>";
        }
        return risultato;
    };
